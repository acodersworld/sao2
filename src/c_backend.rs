...
impl<'a> Renderer<'a> {
    ...
    fn render(mut self) -> String {
        let collection_enabled = self.collection_enabled();
        ...
        if !self.managed_layouts.is_empty() {
            ...
            if collection_enabled {
                ...
                self.render_collection_runtime();
            }
        }
    ...
    fn render_arena_runtime(&mut self) {
        if self.collection_enabled() {
            self.output.push_str("#define SAO2_HEAP_COLLECTION_MODE\n");
        }
        self.output.push_str(ARENA_RUNTIME);
        if self.collection_enabled() {
            self.output.push_str("#undef SAO2_HEAP_COLLECTION_MODE\n");
        }
    }
    ...
    fn render_collection_runtime(&mut self) {
        self.output.push_str(COLLECTION_RUNTIME);
    }
    ...
    fn render_shadow_declarations(&mut self) {
        self.output.push_str(concat!(
            ...
            "static void sao2_shadow_assert_empty(void);\n\n",
        ));
    }
...
typedef struct {
    ...
    uint64_t live_blocks_retained;
    uint64_t dead_blocks_reclaimed;
    uint64_t reclaimed_span_bytes;
    uint64_t resulting_free_blocks;
} sao2_gc_stats;

static sao2_gc_stats sao2_last_gc_stats;
...
static void sao2_arena_runtime_release(void) {
    ...
    if (sao2_gc_active) sao2_compiler_invariant();
    sao2_shadow_assert_empty();
#endif
...
const COLLECTION_RUNTIME: &str = r#"
...
static void sao2_collection_append_free_run(uint64_t offset, uint64_t span,
    ...
    if (offset == 0 || span < (uint64_t)sizeof(sao2_heap_header)
        || head == NULL || tail == NULL) sao2_compiler_invariant();
    sao2_heap_initialize_free(sao2_heap_header_at(offset), span, 0);
    if (*tail == 0) *head = offset;
    else sao2_heap_header_at(*tail)->next_free = offset;
    *tail = offset;
    sao2_last_gc_stats.resulting_free_blocks++;
}

static bool sao2_heap_sweep(uint32_t epoch) {
    uint64_t frontier, offset, next;
    uint64_t free_start = 0, free_span = 0, rebuilt_head = 0, rebuilt_tail = 0;
    sao2_gc_stats stats = {0};

    if (epoch == 0 || !sao2_heap_validate()) return false;
    frontier = sao2_heap_arena.frontier;

    /* Validate every physical step before the first header is rewritten. */
    offset = SAO2_ARENA_INITIAL_CURSOR;
    while (offset < frontier) {
        if (!sao2_heap_block_valid(offset, frontier, &next) || next <= offset)
            return false;
        offset = next;
    }
    if (offset != frontier) return false;

    sao2_last_gc_stats = stats;
    offset = SAO2_ARENA_INITIAL_CURSOR;
    while (offset < frontier) {
        sao2_heap_header *header = sao2_heap_header_at(offset);
        uint32_t state = header->block_state;
        uint64_t span = header->span_size;
        next = offset + span;

        if (state == SAO2_HEAP_BLOCK_ALLOCATED) {
            sao2_last_gc_stats.allocated_blocks_examined++;
            if (header->mark_epoch == epoch) {
                sao2_last_gc_stats.live_blocks_retained++;
                if (free_start != 0) {
                    sao2_collection_append_free_run(free_start, free_span,
                        &rebuilt_head, &rebuilt_tail);
                    free_start = 0;
                    free_span = 0;
                }
            } else {
                sao2_last_gc_stats.dead_blocks_reclaimed++;
                sao2_last_gc_stats.reclaimed_span_bytes += span;
                if (free_start == 0) free_start = offset;
                if (free_span > frontier - free_start - span) sao2_compiler_invariant();
                free_span += span;
            }
        } else if (state == SAO2_HEAP_BLOCK_FREE) {
            if (free_start == 0) free_start = offset;
            if (free_span > frontier - free_start - span) sao2_compiler_invariant();
            free_span += span;
        } else {
    ...
    if (free_start != 0) {
        /* A trailing free run is removed from the frontier, but committed
         * pages remain committed for deterministic reuse. */
        memset(sao2_heap_arena.base + (size_t)free_start, 0, (size_t)free_span);
        sao2_heap_arena.frontier = free_start;
    }
    sao2_heap_arena.free_head = rebuilt_head;
    if (!sao2_heap_validate()) sao2_compiler_invariant();
    return true;
}
...
static sao2_collection_result sao2_collect_for_trigger(uint32_t trigger) {
    ...
    sao2_trace_global_roots(&context);
    if (context.result == SAO2_TRACE_OK) sao2_trace_shadow_roots(&context);
    trace_result = context.result == SAO2_TRACE_OK
        ? sao2_trace_drain(&context)
        : context.result;
    if (trace_result == SAO2_TRACE_OK) {
        result = sao2_heap_sweep(epoch) ? SAO2_COLLECTION_OK : SAO2_COLLECTION_INVALID;
    } else if (trace_result == SAO2_TRACE_SCRATCH_EXHAUSTED) {
        result = SAO2_COLLECTION_TRACE_SCRATCH_EXHAUSTED;
    } else {
        result = SAO2_COLLECTION_INVALID;
    }
